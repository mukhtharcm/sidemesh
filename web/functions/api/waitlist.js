export async function onRequestPost({ request, env }) {
  ...
  const trim = (value, length = 255) => (typeof value === "string" ? value.slice(0, length) : null);
  const headersIn = request.headers;
  const cf = request.cf || {};

  const row = {
    email,
    user_agent: trim(headersIn.get("user-agent")),
    referrer: trim(headersIn.get("referer")),
    ...
    accept_language: trim(headersIn.get("accept-language"), 120),
    ua_platform: trim(headersIn.get("sec-ch-ua-platform")?.replace(/"/g, ""), 40),
    ua_mobile: trim(headersIn.get("sec-ch-ua-mobile"), 8),
