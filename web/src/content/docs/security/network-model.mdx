...
- `GET /healthz` is intentionally unauthenticated for service checks; direct provider failures and probe timeouts return HTTP 503
...
- State directory defaults to `~/.sidemesh`, is restricted to the current user (`0o700`), and can be overridden with `SIDEMESH_STATE_DIR`
