  ...
  Future<void> _openUrl({required bool newTab}) async {
    ...
    try {
      ...
      setState(() {
        ...
        _tabs = [tab, ..._tabs.where((item) => item.id != tab.id)];
      });
      _openTab(tab);
    } catch (error) {
      if (!mounted) return;
                ...
                children: [
                  if (_loading)
                    ...
                    AppSectionHeader(
                      ...
                      title: 'Tabs',
                      subtitle: '${tabs.length} open for this session',
                    ...
                    for (var index = 0; index < tabs.length; index++) ...[
                      _BrowserTabRow(
                        tab: tabs[index],
                        onOpen: () => _openTab(tabs[index]),
                        onClose: () => unawaited(_closeTab(tabs[index])),
                      ),
    ...
    return Column(
      ...
      children: [
        AppSectionHeader(
          ...
          title: title,
          subtitle: 'Enter a local or remote address.',
        ),
        const SizedBox(height: AppSpacing.md),
        TextField(
...
class _BrowserTabRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MeshListRow(
      onTap: running ? onOpen : null,
      framed: false,
      leading: AppIconWell(
        icon: running ? Icons.tab_rounded : Icons.close_rounded,
      ...
      meta: MeshStatusBadge(
        label: running ? 'Open' : _tabStatusLabel(tab.status),
        tone: running ? MeshStatusTone.success : MeshStatusTone.neutral,
      ),
      trailing: IconButton(
        tooltip: 'Close tab',
        onPressed: onClose,
