...
class CreateSessionDraftSeed {
  ...
  factory CreateSessionDraftSeed.fromSession({
    ...
    return CreateSessionDraftSeed(
      ...
      accessMode:
          _trimmedOrNull(policy.accessMode) ??
          _trimmedOrNull(runtime?.accessMode),
...
class _CreateSessionSheetState extends State<CreateSessionSheet> {
  ...
  bool get _configurationIsLoading =>
      _loadingNode || (_supportsAccessModes && _loadingAccessModes);
  ...
  String get _modelLabel {
    ...
    if (_profileToSubmit != null) return 'Use profile model';
    return 'Use default model';
  }

  String get _modelDescription {
    if (!_supportsModels) {
      return 'This agent does not offer model choices here.';
    }
    if (_loadingModels) {
      ...
      return 'Loading models for this machine.';
    }
    ...
    final profileModel = _trimmedOrNull(profile?.model);
    if (profile != null && profileModel != null) {
      return 'This session will use profile ${profile.name}\'s model: $profileModel.';
    }
    final profileName = _profileToSubmit;
    if (profileName != null) {
      return 'Use the profile model, or choose a different one for this session.';
    }
    final defaultModel = _defaultModelEntry;
    if (defaultModel != null) {
      return 'This machine will use ${defaultModel.displayName} unless you choose something else.';
    }
    return 'Leave this unset to use the machine default.';
  }
  ...
  bool? get _networkAccessToSubmit {
    if (_supportsAccessModes) return null;
  ...
  void _selectProvider(String? providerKind) {
    ...
    _accessModesLoadedForProvider = null;
    _coerceForProviderCapabilities();
  }
  ...
  void _handleCwdChanged() {
    ...
    if (_accessModesLoadedForCwd != cwd) {
      setState(() {
        _accessModeCatalog = null;
        _accessModesError = null;
        _accessMode = null;
    ...
    try {
      ...
      setState(() {
        ...
        _modelsError = models.isEmpty
            ? profile == null
                  ? 'No models were returned for this machine.'
                  : 'No models were returned for profile $profile.'
            : null;
  ...
  Future<void> _loadProfiles({bool force = false}) async {
    ...
    if (cwd == null) {
      setState(() {
        _profiles = const <ProviderProfileSummary>[];
        _profilesError = 'Enter a folder to load saved profiles.';
        _defaultProfileName = null;
        _profilesLoadedForCwd = null;
      });
      return;
  ...
  Future<void> _chooseModel() async {
    ...
    _applyModelPickerResult(result);
  }

  void _applyModelPickerResult(_ModelPickerResult result) {
    setState(() {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final session = await widget.api.createSession(
        ...
        accessMode: _supportsAccessModes ? _accessMode : null,
  ...
  bool get _draftUsesBroadPermissions =>
      (_supportsAccessModes && (_selectedAccessMode?.isDangerous ?? false)) ||
      (!_supportsAccessModes &&
  ...
  ProviderAccessModeSummary? get _selectedAccessMode {
    ...
    return providerAccessModeById(catalog, _accessMode);
  }

  Future<void> _selectAccessMode(ProviderAccessModeSummary mode) async {
    if (!mode.enabled) return;
    if (mode.id == _accessMode) return;
    ...
    return AppChoiceRow(
      ...
      footer: _badges.isEmpty
          ...
          : Wrap(
              ...
              children: [
                for (final badge in _badges) MeshInlineBadge(label: badge),
              ],
            ),
    );
  }
                  ...
                  itemBuilder: (context, index) {
                    if (query.isEmpty && index == 0) {
                      ...
                      return _ModelPickerTile(
                        ...
                        onTap: () =>
                            _select(context, const _ModelPickerResult()),
                      );
                    }
                    ...
                    return _ModelPickerTile(
                      ...
                      onTap: () =>
                          _select(context, _ModelPickerResult(model: model)),
                    );
                  },
                ),
        ),
...
class _ModelPickerTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AppChoiceRow(
      ...
      selected: selected,
      onTap: onTap,
      footer: model == null && badges.isEmpty
          ? null
          : Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                if (model != null && badges.isNotEmpty)
                  const SizedBox(height: AppSpacing.sm),
                if (badges.isNotEmpty)
                  Wrap(
                    ...
                    runSpacing: AppSpacing.sm,
                    children: [
                      for (final badge in badges) MeshInlineBadge(label: badge),
                    ],
                  ),
              ],
            ),
...
String _describeProviderProfile(ProviderProfileSummary profile) {
  final parts = <String>[
    ...
    if ((profile.sandboxMode ?? '').isNotEmpty)
      '${_sandboxModeLabel(profile.sandboxMode!)} access',
  ...
  if (parts.isEmpty) {
    return 'Saved setup for this agent.';
  }
...
String _profileModelInheritDescription(
  ...
  if (profileModel != null) {
    return 'Use the model from profile $profileName: $profileModel.';
  }
  return 'Use the default model from profile $profileName.';
}
...
class _DirectoryPickerSheetState extends State<_DirectoryPickerSheet> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final isWorkspaceError =
          msg.contains('workspace') || msg.contains('outside');
      setState(() {
        _loading = false;
