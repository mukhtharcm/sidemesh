...
export function reconcileAcpReplay(local: StoredSessionItem[], replay: StoredSessionItem[]): StoredSessionItem[] {
  ...
  const result = replay.map((item, index): StoredSessionItem => {
    ...
    const candidate = sameId ?? (prefixMatches ? local[index] : undefined);
    ...
    if (matches) {
      ...
      return { ...withItemMetadata(item, { id: candidate.value.id, createdAt: candidate.value.createdAt }), authority: "cache" };
    }
    return { ...item, authority: "cache" };
  });
